...
router.post('/updateFile', async (req, res) => {
	...
	console.log(req.body);
	try {
		const updateFile = await file.findByIdAndUpdate({ _id: req.body.id }, req.body, { new: true });
...
router.post('/deleteFile', async (req, res) => {
	try {
		const deleteFile = await file.findByIdAndDelete({ _id: req.body.fileid }, req.body, {
			new: true,
		});
...
router.put('/putFile/:id', async (req, res) => {
	try {
		const updatedFiles = await file.findByIdAndUpdate(req.params.id, req.body, { new: true });
